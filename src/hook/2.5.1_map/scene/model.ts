...
const drawPolygoneMesh = (polygon, color: string) => {
  ...
  return new THREE.Mesh(geometry, material); //mesh;
};
// 解析数据
const operationData = (data) => {
  data.features.forEach((f) => {
    ...
    const color = ["重庆市", "上海市"].includes(f.properties.name)
      ? "green"
      : "red";
...
const addMeshToProvince = (province, coordinate, color) => {
  coordinate.forEach((polygon) => {
    ...
    provinceArr.push(drawPolygoneMesh(polygon, color));
...
export { model, provinceArr, circleArr, moveSpotsArr };
